...
def calculer_moyenne_globale(data):
    moyennes_globales = {}

    # Parcours de chaque colonne (caractéristique) dans les données
    for key, valeurs in data.items():
        if key != "Numero":  # Ignorer la colonne "ComplexitePercu"
            moyennes_globales[key] = np.mean(valeurs) if len(valeurs) > 0 else np.nan

    return moyennes_globales
...
def main(dossier_path_Precision, dossier_path_EDA, PerceptionReproduciton):
    ...
    for complexite in range(1, 27):
        print(f"NumeroSequence {complexite}:")
    ...
    """
    moyennes = calculer_moyenne_globale(data)
    print(f"{moyennes}")
    """
